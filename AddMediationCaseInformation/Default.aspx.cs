...
System.Web.UI.Page
{
                    ...
                    if (datareader.FieldCount > 0)
                    {
                        ...
                        if (datareader.Read())
                        {
                            ...
                            if (datareader["ConfResolutionDate"] != DBNull.Value)
                            {
                                ...
                                if (new[] { 7, 8 }.Contains(lstCertificateNoticeTypeReason.SelectedIndex))//_obd 3/3/15
                                {
                                    btnPrintCertificate.Visible = false;
                                    btnReprintCertificate.Visible = false;
                                    btnPrintMediationInvoice.Visible = false;
    ...
    protected void btnReprintCertificate_Click(object sender, EventArgs e)
    {
        ...
        if (txtCertificateNoticePrintDate.Text != "")
        {
            ...
            int certID = lstCertificateNoticeTypeReason.SelectedIndex;
            if (certID > 8)
            {
                certID++;
            }
            ResponseHelper.Redirect("~/SpecificTask/dupefees.aspx?CaseId=" + txtMedCaseID.Text + "&CertID=" + certID, "_blank", "menubar=0,resizable=yes,width=363em,height=220em");
        }
        else
        {
    ...
    void printReport()
    {
        ...
        ReportDocument crystalReport = new ReportDocument();
        crystalReport.Load(Server.MapPath("~/Reports/Certificates/" + certID + ".rpt"));
        ...
        DataTable data = Mediationobj.GetAllCertificatesDetails(Convert.ToInt32(Session["MediationCaseID"].ToString()), certID);
        crystalReport.SetDataSource(data);
        lstCertificateNoticeTypeReason.Enabled = false;
        if (txtCertificateNoticePrintDate.Text == "")
        {
